...
**Resolution:** the frontmost app is read before output processing (`FrontmostAppResolver`),
...
`AppIdentityMatching`. It is checked again before injection; a changed app holds
the result for explicit copying. This does not identify fields or tabs within
the same app.
...
**Deferred:** user-authored named styles, concise rewriting, multilingual intent
qualification, structured-command rewriting, and browser-tab matching.
