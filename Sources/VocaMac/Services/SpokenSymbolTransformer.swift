    ...
    private static func effectiveEscapes(
        ...
        return Set(
            candidates.filter { candidate in
                // At line start the baseline has no filename stem. Keeping
                // "literally" would itself supply one (literally.json).
                if tiers.contains(.tierA), candidate + 2 < words.count,
                   core(of: words[candidate + 1]).lowercased() == "dot",
                   knownExtensions.contains(core(of: words[candidate + 2]).lowercased()) {
                    return true
                }
